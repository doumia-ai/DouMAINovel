...
class AIService:
    ...
    def __init__(
        ...
        self.enable_key_pool = enable_key_pool

        # 保存原始 API Key 和 Base URL
        self._base_api_key = api_key
    ...
    def _get_current_key(self) -> str:
        """获取当前应使用的 API Key（支持轮询）"""
        if not self.enable_key_pool or not self.user_id:
            return self._base_api_key

        from app.services.key_pool_manager import key_pool_manager
        return key_pool_manager.get_next_key(
            user_id=self.user_id,
            provider=self.api_provider,
            base_url=self._api_base_url or "",
            model=self.default_model,
            fallback_key=self._base_api_key
        )

    def _create_provider_with_key(self, key: str, provider: Optional[str] = None) -> BaseAIProvider:
        """使用指定的 Key 创建 Provider"""
        p = provider or self.api_provider
        if p == "openai":
            base_url = self._api_base_url or app_settings.openai_base_url or "https://api.openai.com/v1"
            client = OpenAIClient(key, base_url, self.config)
            return OpenAIProvider(client)
        if p == "anthropic":
            base_url = self._api_base_url or app_settings.anthropic_base_url
            client = AnthropicClient(key, base_url, self.config)
            return AnthropicProvider(client)
        if p == "gemini":
            client = GeminiClient(key, self._api_base_url, self.config)
            return GeminiProvider(client)
        raise ValueError(f"Provider {p} 不支持")

    def _get_provider(self, provider: Optional[str] = None) -> BaseAIProvider:
        """获取对应的 Provider（支持 Key 池轮询）"""
        p = provider or self.api_provider

        # 如果启用了 Key 池，使用轮询的 Key 创建新 Provider
        if self.enable_key_pool and self.user_id:
            current_key = self._get_current_key()
            if current_key and current_key != self._base_api_key:
                return self._create_provider_with_key(current_key, p)
